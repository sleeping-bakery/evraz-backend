...
internal abstract class DependencyPromptGenerator
{
    public static string GeneratePromptFromZip(ZipArchive archive)
    {
        ...
        foreach (var content in csprojFiles.Select(ExtractFileContent)) AppendCsprojDependencies(content, prompt);

        foreach (var content in packagePropsFiles.Select(ExtractFileContent)) AppendPackagePropsDependencies(content, prompt);

        foreach (var content in globalJsonFiles.Select(ExtractFileContent)) AppendGlobalJsonDependencies(content, prompt);

        return prompt.ToString();
    }

    private static string ExtractFileContent(IArchiveEntry entry)
    {
    ...
    private static void AppendCsprojDependencies(string content, StringBuilder prompt)
    {
        ...
        foreach (var reference in packageReferences) prompt.AppendLine($"- Пакет: {reference.Package}, Версия: {reference.Version}");
        ...
        foreach (var path in hintPathsList) prompt.AppendLine($"  - {path}");
    }
    ...
    private static void AppendPackagePropsDependencies(string content, StringBuilder prompt)
    {
        ...
        foreach (var package in packageVersions) prompt.AppendLine($"- Главный пакет: {package.Package}, Версия: {package.Version}");
    }
    ...
    private static void AppendGlobalJsonDependencies(string content, StringBuilder prompt)
    {
        ...
        if (!string.IsNullOrWhiteSpace(sdkVersion)) prompt.AppendLine($"- SDK Версия: {sdkVersion}");
    }
}